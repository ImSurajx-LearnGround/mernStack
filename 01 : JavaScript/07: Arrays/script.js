...
let xmarks = new Array(); // creating array
...
for (let i = 0; i < arr.length; i++) console.log(arr[i]);
let xnewarr = arr.slice(0, 3); // extract value from array & create new array with those values, no changes in main array
...
for (let i = 0; i < arr.length; i++) console.log(arr[i]);
arr.indexOf(4); // tell index of element


let xarray = [11,62, 3, 4, 25]; 
...
let ynewarr = array.map(function(val){
    if(val > 10) return 12;
    else return 5;
})
for (let i = 0; i < newarr.length; i++) console.log(newarr[i]);
...
// destructring
let [ax,bx,,d,,f] = arr; // copies value from array
console.log(a,b,d,f);
...
let xsum = arrNum.reduce( (accumulator, val) => {
...
// ques: 
let ymarks = [90, 45, 78, 32, 56, 88, 29];
